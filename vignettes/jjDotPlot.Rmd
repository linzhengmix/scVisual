---
title: "jjDotPlot: Dot Plot for Gene Expression"
author: "mixfruit"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{jjDotPlot: Dot Plot for Gene Expression}
...
The `jjDotPlot` function creates dot plots for visualizing gene expression across clusters. Dot plots are effective for displaying both the average expression level and the percentage of cells expressing a gene within each cluster.
...
While `jjDotPlot` typically works with Seurat objects, we can demonstrate its usage with the built-in marker data:
...
You can customize the dot plot by adjusting color scheme, dot size, and grouping:
...
- Consider using `features` to select a manageable number of genes for clarity
